...
type ExchangeConfig struct {
	...
	Enabled   bool   `json:"enabled"`
	APIKey    string `json:"apiKey"`    // For Binance/Gate.io: API Key; For Hyperliquid: Agent Private Key (should have ~0 balance)
	SecretKey string `json:"secretKey"` // For Binance/Gate.io: Secret Key; Not used for Hyperliquid
...
func (d *Database) UpdateAIModel(userID, id string, enabled bool, apiKey, customAPIURL, customModelName string) error {
	...
	if err == nil {
		// 找到了现有配置（精确匹配 ID），更新它
		// 🔒 安全特性：只在apiKey非空时才更新，否则保留现有值
		updateSQL := "UPDATE ai_models SET enabled = ?, custom_api_url = ?, custom_model_name = ?, updated_at = datetime('now')"
		args := []interface{}{enabled, customAPIURL, customModelName}

		if apiKey != "" {
			// 只有当apiKey非空时才更新
			updateSQL += ", api_key = ?"
			args = append(args, d.encryptSensitiveData(apiKey))
		}
	...
	if provider == id && (provider == "deepseek" || provider == "qwen" || provider == "openai" || provider == "doubao") {
		// id 本身就是 provider
		provider = id
	} else {
