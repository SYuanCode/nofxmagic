...
func (l *DecisionLogger) AnalyzePerformance(lookbackCycles int) (*PerformanceAnalysis, error) {
	...
	if err == nil && len(allRecords) > len(records) {
		// 构建临时持仓状态，用于找出未平仓的持仓
		tempPositions := make(map[string]map[string]interface{})

		// 遍历所有记录，按时间顺序处理开仓和平仓动作
		// 注意：GetLatestRecords返回的记录是按时间倒序的，所以需要反转
		for i := len(allRecords) - 1; i >= 0; i-- {
			...
			for _, action := range record.Decisions {
				...
				case "open_long", "open_short":
					// 开仓：添加或更新持仓记录
					tempPositions[posKey] = map[string]interface{}{
						"side":               side,
						...
						"accumulatedPnL":     0.0,
						"partialCloseCount":  0,
						"partialCloseVolume": 0.0,
					}

				case "close_long", "close_short", "auto_close_long", "auto_close_short":
					// 完全平仓：移除持仓记录
					if _, exists := tempPositions[posKey]; exists {
						delete(tempPositions, posKey)
					}

				case "partial_close":
					// 部分平仓：更新持仓记录的剩余数量
					if pos, exists := tempPositions[posKey]; exists {
						remainingQty, _ := pos["remainingQuantity"].(float64)
						if remainingQty == 0 {
							remainingQty = pos["quantity"].(float64)
						}
						...
						if remainingQty <= 0.0001 {
							...
							// 更新剩余数量
							pos["remainingQuantity"] = remainingQty
							accumulatedPnL, _ := pos["accumulatedPnL"].(float64)
							partialCloseCount, _ := pos["partialCloseCount"].(int)
							partialCloseVolume, _ := pos["partialCloseVolume"].(float64)

							// 计算本次部分平仓的盈亏
							openPrice := pos["openPrice"].(float64)
							var pnl float64
							if side == "long" {
								pnl = action.Quantity * (action.Price - openPrice)
							} else {
								pnl = action.Quantity * (openPrice - action.Price)
							}

							// 更新累积盈亏和部分平仓统计
							pos["accumulatedPnL"] = accumulatedPnL + pnl
							pos["partialCloseCount"] = partialCloseCount + 1
							pos["partialCloseVolume"] = partialCloseVolume + action.Quantity
						}
					}
				}
			}
		...
		// 将未平仓的持仓复制到openPositions
		for key, pos := range tempPositions {
	...
	for _, record := range records {
		for _, action := range record.Decisions {
			...
			if action.Action == "partial_close" {
				// 從 openPositions 中查找當前symbol的持倉方向
