...
// sk-b3c7aed071b5430188aab05e4c52b725
// LeBaloKIXKBtO3NbksYu3UlM3TTNrc8LgLtGzrxMjMeVacjVKhyeQFrJUftofE7f
// 7TO0Qq4p3VzE11azgSQxkJakm8xNsgC88Nf2ru5e0SrRaJMmr7eAaibltxCWzhdP
// ConfigFile 配置文件结构，只包含需要同步到数据库的字段
// TODO 现在与config.Config相同，未来会被替换， 现在为了兼容性不得不保留当前文件

// 豆包：be576603-90c4-4372-83d1-6a595057c405
...
func main() {
	...
	// 初始化Logger
	if err := logger.InitFromLogConfig(cfgForAI.Log); err != nil {
		log.Printf("⚠️  初始化Logger失败: %v", err)
	} else {
		log.Println("✅ Logger初始化成功")
	}
	...
	// 1. 优先从环境变量 AIFOX_BACKEND_PORT 读取
	if envPort := strings.TrimSpace(os.Getenv("AIFOX_BACKEND_PORT")); envPort != "" {
		if port, err := strconv.Atoi(envPort); err == nil && port > 0 {
			apiPort = port
			log.Printf("🔌 使用环境变量端口: %d (AIFOX_BACKEND_PORT)", apiPort)
		} else {
			log.Printf("⚠️  环境变量 AIFOX_BACKEND_PORT 无效: %s", envPort)
		}
	} else if apiPortStr != "" {
