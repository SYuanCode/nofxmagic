...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		protected := api.Group("/", s.authMiddleware())
		{
			...
			protected.GET("/trades", s.handleTradeHistory)
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	if exchangeCfg == nil {
		...
		case "gate":
			tempTrader = trader.NewGateFuturesTrader(exchangeCfg.APIKey, exchangeCfg.SecretKey, userID)
		default:
	...
	log.Printf("🔧 DEBUG: 准备调用 LoadTraderByID")
	err = s.traderManager.LoadTraderByID(s.database, userID, traderID)
	if err != nil {
		log.Printf("⚠️ 加载新交易员到内存失败: %v", err)
		// 这里不返回错误，因为交易员已经成功创建到数据库
	}
	log.Printf("🔧 DEBUG: LoadTraderByID 完成")
...
func (s *Server) handleStartTrader(c *gin.Context) {
	...
	// 检查交易员是否已加载到内存，如果没有则加载
	trader, err := s.traderManager.GetTrader(traderID)
	if err != nil {
		// 尝试从数据库加载交易员到内存
		log.Printf("🔧 交易员 %s 未在内存中，尝试从数据库加载...", traderID)
		err := s.traderManager.LoadTraderByID(s.database, userID, traderID)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "加载交易员到内存失败", "err": err.Error()})
			return
		}
		// 重新获取交易员
		trader, err = s.traderManager.GetTrader(traderID)
		if err != nil {
			c.JSON(http.StatusNotFound, gin.H{"error": "交易员不存在", "err": err.Error()})
			return
		}
	}
...
func (s *Server) handleAccount(c *gin.Context) {
	...
	err = s.traderManager.LoadUserTraders(s.database, userID)
	if err != nil {
		log.Printf("⚠️ 加载用户 %s 的交易员失败: %v", c.GetString("user_id"), err)
	}
	trader, err := s.traderManager.GetTrader(traderID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
...
func (s *Server) handleTradeHistory(c *gin.Context) {
	...
	// 调用交易员的GetTradeHistory方法获取交易历史
	trades, err := trader.GetTradeHistory(symbol, limit)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
