...
type AsterTrader struct {
	...
	// 记录代币最后平仓时间，用于限制半小时内不能再次开仓
	lastCloseTime map[string]time.Time
	closeTimeMu   sync.RWMutex
}
...
func (t *AsterTrader) OpenLong(symbol string, quantity float64, leverage int) (map[string]interface{}, error) {
	...
	// 检查该代币是否在半小时内已平仓
	t.closeTimeMu.RLock()
	if closeTime, exists := t.lastCloseTime[symbol]; exists {
		t.closeTimeMu.RUnlock()
		if time.Since(closeTime) < 30*time.Minute {
			return nil, fmt.Errorf("❌ %s 在 %.0f 分钟前刚平仓，拒绝开仓。请等待至少 30 分钟后再试",
				symbol, time.Since(closeTime).Minutes())
		}
	} else {
		t.closeTimeMu.RUnlock()
	}
