...
type AutoTraderConfig struct {
	...
	// 交易平台选择
	Exchange string // "binance", "bybit", "hyperliquid", "aster", "lighter" 或 "gate"
...
type AutoTrader struct {
	...
	userID                string               // 用户ID
	lastCloseTime         map[string]time.Time // 记录每个代币的最后平仓时间 (symbol -> timestamp)
	lastCloseTimeMutex    sync.RWMutex         // 平仓时间缓存读写锁
}
...
func NewAutoTrader(config AutoTraderConfig, database interface{}, userID string) (*AutoTrader, error) {
	...
	case "gate":
		...
		trader = NewGateFuturesTrader(config.GateAPIKey, config.GateSecretKey, userID)
	default:
...
func (at *AutoTrader) runCycle() error {
	...
	// 4. 检查并执行收益率策略：如果收益率超过80%，然后降到40%以下则立即平仓
	for _, pos := range ctx.Positions {
		// 更新峰值收益率缓存
		posKey := pos.Symbol + "_" + pos.Side
		at.peakPnLCacheMutex.Lock()
		if pos.UnrealizedPnLPct > at.peakPnLCache[posKey] {
			at.peakPnLCache[posKey] = pos.UnrealizedPnLPct
			log.Printf("📈 持仓 %s %s 峰值收益率更新: %.2f%%", pos.Symbol, pos.Side, pos.UnrealizedPnLPct)
		}
		peakPnlPct := at.peakPnLCache[posKey]
		at.peakPnLCacheMutex.Unlock()

		// 检查策略条件：峰值超过80%，当前低于40%
		if peakPnlPct >= 80.0 && pos.UnrealizedPnLPct < 40.0 {
			log.Printf("⚠️  触发收益率保护策略：持仓 %s %s 峰值收益率 %.2f%%，当前降到 %.2f%%，执行自动平仓",
				pos.Symbol, pos.Side, peakPnlPct, pos.UnrealizedPnLPct)

			// 创建自动平仓决策
			autoCloseDecision := &decision.Decision{
				Symbol: pos.Symbol,
				Action: "close_" + strings.ToLower(pos.Side),
				Reasoning: fmt.Sprintf("收益率保护策略：峰值 %.2f%%，当前 %.2f%%，触发自动平仓",
					peakPnlPct, pos.UnrealizedPnLPct),
			}

			// 执行平仓
			actionRecord := logger.DecisionAction{
				Action:   autoCloseDecision.Action,
				Symbol:   autoCloseDecision.Symbol,
				Quantity: 0,
				Leverage: pos.Leverage,
			}

			if err := at.executeDecisionWithRecord(autoCloseDecision, &actionRecord); err != nil {
				log.Printf("❌ 自动平仓失败: %v", err)
			} else {
				// 平仓成功后清除峰值缓存
				at.peakPnLCacheMutex.Lock()
				delete(at.peakPnLCache, posKey)
				at.peakPnLCacheMutex.Unlock()
				log.Printf("✅ 自动平仓成功: %s %s", pos.Symbol, pos.Side)
			}
		}
	}
...
func (at *AutoTrader) executeOpenLongWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	if closeTime, exists := at.lastCloseTime[decision.Symbol]; exists {
		at.lastCloseTimeMutex.RUnlock()
		if time.Since(closeTime) < 30*time.Minute {
			return fmt.Errorf("❌ %s 在 %.0f 分钟前刚平仓，拒绝开仓。请等待至少 30 分钟后再试",
				decision.Symbol, time.Since(closeTime).Minutes())
		}
	} else {
		at.lastCloseTimeMutex.RUnlock()
	}
	...
	if price, ok := order["price"].(float64); ok {
		...
		// 如果没有实际成交价格，使用最新市场价格
		log.Printf("  ⚠ 订单中没有实际成交价格，使用最新市场价格")
		actualPrice = marketData.CurrentPrice
	}

	// 更新actionRecord.Price为实际成交价格
	actionRecord.Price = actualPrice

	log.Printf("  ✓ 开仓成功，订单ID: %v, 数量: %.4f, 成交价格: %.4f", order["orderId"], quantity, actualPrice)
...
func (at *AutoTrader) executeCloseLongWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	// 获取平仓前的持仓信息，用于计算盈亏和收益率
	positions, _ := at.trader.GetPositions()
	var entryPrice, unrealizedPnl, leverage, positionAmt float64
	var found bool
	for _, pos := range positions {
		if pos["symbol"] == decision.Symbol && pos["side"] == "long" {
			...
			if lev, ok := pos["leverage"].(float64); ok {
				...
				leverage, _ = strconv.ParseFloat(levStr, 64)
			}
			found = true
			break
		}
	...
	if !found {
		return fmt.Errorf("未找到 %s 的多仓持仓", decision.Symbol)
	}

	// 计算保证金和收益率
	// 保证金 = 仓位价值 / 杠杆
	positionValue := marketData.CurrentPrice * math.Abs(positionAmt)
	margin := positionValue / leverage
	pnlPercentage := 0.0
	if margin > 0 {
		pnlPercentage = (unrealizedPnl / margin) * 100
	}
	...
	var reason string
	if leverage >= 30 {
		// 30倍杠杆以上：-20% 到 30% 之间禁止平仓
		if pnlPercentage >= -20 && pnlPercentage <= 30 {
			isForbidden = true
			reason = fmt.Sprintf("30倍杠杆以上，收益率 (%.2f%%) 在 -20%% 到 30%% 之间", pnlPercentage)
		}
	} else {
		// 其他杠杆：-10% 到 15% 之间禁止平仓
		if pnlPercentage >= -10 && pnlPercentage <= 15 {
			isForbidden = true
			reason = fmt.Sprintf("其他杠杆，收益率 (%.2f%%) 在 -10%% 到 15%% 之间", pnlPercentage)
		}
	}

	if isForbidden {
		log.Printf("  ❌ 禁止平仓: %s", reason)
		return fmt.Errorf("禁止平仓: %s", reason)
	}
	...
	// 记录平仓时间，用于限制半小时内不能再次开仓
	now := time.Now()
	at.lastCloseTimeMutex.Lock()
	at.lastCloseTime[decision.Symbol] = now
	at.lastCloseTimeMutex.Unlock()
	log.Printf("  ⏱️  记录 %s 平仓时间: %s (半小时内将禁止再次开仓)", decision.Symbol, now.Format("2006-01-02 15:04:05"))
...
func (at *AutoTrader) executeCloseShortWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	if !found {
		return fmt.Errorf("未找到 %s 的空仓持仓", decision.Symbol)
	}
...
func (at *AutoTrader) GetTradeHistory(symbol string, limit int) ([]map[string]interface{}, error) {
	if at.trader == nil {
		return nil, fmt.Errorf("交易器未初始化")
	}

	// 调用底层交易器的GetTradeHistory方法获取交易历史
	return at.trader.GetTradeHistory(symbol, limit)
}
...
func (at *AutoTrader) checkPositionDrawdown() {
	...
	for _, pos := range positions {
		...
		// 检查平仓条件：收益大于30%且回撤超过40%
		if currentPnLPct > 30.0 && drawdownPct >= 40.0 {
