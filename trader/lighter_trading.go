...
func (t *LighterTrader) SetStopLoss(symbol string, positionSide string, quantity, stopPrice float64) error {
	...
	// 创建带ReduceOnly的限价止损单，确保只会平仓不会开新仓
	orderID, err := t.CreateOrderWithOptions(symbol, side, quantity, stopPrice, "limit", true)
	if err != nil {
		return fmt.Errorf("设置止损失败: %w", err)
	}

	log.Printf("✓ LIGHTER - 止损已设置: %.2f (side: %s, orderID: %s, symbol: %s)", stopPrice, side, orderID, symbol)
	return nil
}
...
func (t *LighterTrader) SetTakeProfit(symbol string, positionSide string, quantity, takeProfitPrice float64) error {
	...
	// 创建带ReduceOnly的限价止盈单，确保只会平仓不会开新仓
	orderID, err := t.CreateOrderWithOptions(symbol, side, quantity, takeProfitPrice, "limit", true)
	if err != nil {
		return fmt.Errorf("设置止盈失败: %w", err)
	}

	log.Printf("✓ LIGHTER - 止盈已设置: %.2f (side: %s, orderID: %s, symbol: %s)", takeProfitPrice, side, orderID, symbol)
	return nil
}
