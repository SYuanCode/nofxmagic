...
func (t *LighterTrader) CreateOrder(symbol, side string, quantity, price float64, orderType string) (string, error) {
	return t.CreateOrderWithOptions(symbol, side, quantity, price, orderType, false)
}

// CreateOrderWithOptions 创建带选项的订单（支持设置ReduceOnly）
func (t *LighterTrader) CreateOrderWithOptions(symbol, side string, quantity, price float64, orderType string, reduceOnly bool) (string, error) {
	...
	log.Printf("✓ LIGHTER订单已创建 - ID: %s, Symbol: %s, Side: %s, Qty: %.4f, ReduceOnly: %v",
		orderResp.OrderID, symbol, side, quantity, reduceOnly)
