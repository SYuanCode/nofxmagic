...
// GetTradeHistory 获取交易历史记录（Lighter实现）
func (t *LighterTraderV2) GetTradeHistory(symbol string, limit int) ([]map[string]interface{}, error) {
	if limit <= 0 {
		limit = 500 // 默认获取500条
	}

	// Lighter API 目前不支持直接获取交易历史，返回空列表
	// 后续可以添加Lighter API的交易历史实现
