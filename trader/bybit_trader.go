...
// GetTradeHistory 获取交易历史记录（Bybit实现）
func (t *BybitTrader) GetTradeHistory(symbol string, limit int) ([]map[string]interface{}, error) {
	if limit <= 0 {
		limit = 500 // 默认获取500条
	}

	// Bybit API 目前不支持直接获取交易历史，返回空列表
	// 后续可以添加Bybit API的交易历史实现
